...
namespace myLight {
	...
	struct {
		...
		*/
		...
		std::shared_ptr<Mesh<Vertex>> pointLightShape = nullptr;
	}DEFAULT;
	...
	struct Light {
		...
		std::shared_ptr<Mesh<Vertex>> p_obj;
	...
	static PointLight* newPointLight(glm::vec3& pos) {
		PointLight* light = new PointLight(pos);
		if (!DEFAULT.pointLightShape.get()) {
			DEFAULT.pointLightShape = std::make_shared<Mesh<Vertex>>(Shape::makeCube());
		}
		light->p_obj = DEFAULT.pointLightShape;
		return light;
	}
