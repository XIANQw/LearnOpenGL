...
vec4 computeDirLightWithShadow(vec3 samplingDiff, vec3 samplingSpec){
    ...
    vec3 diffuse = diff * dirLight.Color * samplingDiff;
    // Specular
    vec3 viewDir = normalize(cameraPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    vec3 specular = dirLight.Color * spec  * samplingSpec;
    // º∆À„“ı”∞
...
void main()
{
    ...
    if(compare){
        ...
        FragColor = computeDirLightWithShadow(samplingDiffRes, samplingSpecRes);
    }
